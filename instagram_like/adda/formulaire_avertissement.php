...
<body class="bg-transparent">
        ...
        <div class="signup-form-wrapper" style="margin-top:5%" >
            <h1 class="create-acc text-center">Avertir cet utilisateur</h1>
            <div class="signup-inner text-center">
                <h3 class="title">Motifs</h3>
                <form class="signup-inner--form" method="post" action="./formulaire_avertissement.php">
                    <div class="row">
                        
                        <div class="col-12">
                            <input type="text" class="single-field" name="motifs" value="Vous avez reçu un avertissement concernant un de vos postes">
            ...
            if (!empty($_POST['motifs'])) {
                ...
                if(isset($_POST['post_id']) && isset($_POST['id_utilisateur'])) {
                    ...
                    if (mysqli_query($conn, $sql_notification)) {
                        ...
                        $sql_update_post = "UPDATE poste SET averti = 1 WHERE id = $post_id";
                        mysqli_query($conn, $sql_update_post);
                        echo json_encode(array("success" => true));
                    } else {
                        // Erreur lors de l'insertion
                        echo json_encode(array("success" => false, "message" => "Erreur lors de l'insertion de la notification : " . mysqli_error($conn)));
                    }
                } else {
                    // Variables post_id et id_utilisateur non définies
                    echo json_encode(array("success" => false, "message" => "Les variables post_id et id_utilisateur ne sont pas définies."));
                }
            } else {
                // Motif non spécifié
                echo json_encode(array("success" => false, "message" => "Veuillez donner un motif"));
            }
        }
    ?>
